...
def physics_analysis_system_prompt() -> str:
    ...
    return (
        ...
        "      \"element_type\": \"rigid_body\" | \"pendulum_bob\" | \"spring_constraint\" | \"spring_launcher\" | \"rope_constraint\" | \"pivot\" | \"anchor\" | \"surface\" | \"conveyor_belt\" | \"pulley_fixed\",\n"
        "      \"is_concave\": boolean,\n"
        ...
        "  * rope_constraint: 绳索约束，连接两个物体，类似弹簧但不可压缩只能拉伸，前端会提示用户选择两个连接点\n"
        ...
        "  * pulley_fixed: 定滑轮（固定位置，可旋转，绳子可绕过，改变力的方向）\n"
        "- 元素角色（role）：\n"
        ...
        "- 定滑轮（pulley_fixed）识别与参数：\n"
        "  * 视觉特征：圆形或圆盘状物体，有明显的轴心或悬挂点，通常固定在天花板、支架或墙上\n"
        "  * 功能：固定位置，可绕轴旋转，绳子可以绕过滑轮改变方向，不提供省力效果\n"
        "  * 类型设置：element_type = \"pulley_fixed\", role = \"static\"（因为位置固定）\n"
        "  * 参数：pulley_radius（滑轮半径，像素），若无法确定则填 null\n"
        "  * 约束设置：needs_pivot = false, needs_second_pivot = false（滑轮本身不需要选择支点）\n"
        "  * 绳子可以绕过定滑轮，绳子本身应识别为 rope_constraint，绳子的连接点可能是滑轮、固定点或重物\n"
        "  * 区分滑轮与普通圆形物体：\n"
        "    - 滑轮：有明显的轴心、凹槽（用于放置绳子）、可能有辐条\n"
        "    - 普通圆形物体（球体）：没有轴心、没有凹槽、实心\n"
        "- 约束关系（constraints，重要！）：\n"
        "  * needs_pivot: 该元素是否需要用户选择第一个支点/连接点\n"
        "    - pendulum_bob: true（需要选择悬挂支点）\n"
        "    - spring_constraint/spring_launcher/rope_constraint: true（需要选择第一个连接点）\n"
        "    - rigid_body/pivot/anchor/surface: false（不需要选择支点）\n"
        "  * needs_second_pivot: 该元素是否需要用户选择第二个支点/连接点\n"
        "    - spring_constraint/spring_launcher/rope_constraint: true（需要选择第二个连接点）\n"
        "    - 其他类型: false\n"
        ...
        "  * pulley_radius: 滑轮半径（像素），用于定滑轮和动滑轮\n"
        "- 单摆/复合摆识别要点：\n"
        ...
        "- 绳索系统识别要点：\n"
        "  * 绳索本身需要分割识别，作为独立元素，role 设为 \"constraint\"（约束类型，不是物体）\n"
        "  * 绳索约束（rope_constraint）：\n"
        "    - 特征：绳索、绳子、缆绳、线缆等柔性连接物，连接两个物体，只能拉伸不能压缩\n"
        "    - 设置：element_type = \"rope_constraint\", role = \"constraint\", needs_pivot = true, needs_second_pivot = true\n"
        "    - pivot_prompt: \"请选择绳子的第一个连接点\"\n"
        "    - second_pivot_prompt: \"请选择绳子的第二个连接点\"\n"
        "    - 参数：rope_length_m（绳长），constraint_stiffness（刚度，0.8-1.0）\n"
        "    - 约束类型：constraint_type = \"rope\"\n"
        "  * 注意：绳索只能拉伸不能压缩，与弹簧的区别在于绳索松弛时不施加力\n"
